...
async function run() {
    ...
    const emp112 = data.find(r => String(r.ID) === '112' || String(r['Employee ID']) === '112');
    
    if(!emp112) {
        console.error("Could not find Employee 112");
        process.exit(1);
    }
    ...
    // Cycle 1 to 10
    for(let i=1; i<=10; i++) {
        let onStartKey = Object.keys(emp112).find(k => k.includes(`DateON${i}`));
        let onEndKey = Object.keys(emp112).find(k => k.includes(`DateOFF${i}`) || k.includes(`DateOff${i}`));
        
        if (emp112[onStartKey] && typeof emp112[onStartKey] === 'number') {
            let start = parseExcelDate(emp112[onStartKey]);
            let end = parseExcelDate(emp112[onEndKey]);
            records.push({
            ...
            let dayOffKey = Object.keys(emp112).find(k => k.includes(`DayOFF${i}`) || k.includes(`DayOff${i}`));
            let leaveDuration = emp112[dayOffKey];
    ...
    try {
        await db.collection("employees").doc("112").update({ Rotations: records });
        console.log("Successfully restored rotations to Firebase for Employee 112!");
    } catch(e) {
