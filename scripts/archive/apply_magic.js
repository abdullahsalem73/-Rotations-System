...
// 1. Remove the old modal HTML and old script
const modalStart = '<div id="globalMapModal"';
const scriptEnd = '</script>\n\n</body>'; // The old one was placed right before </body>

const startIdx = html.lastIndexOf(modalStart);
if (startIdx !== -1) {
...
// 2. Inject the Magic Global Listener at the end of <head>
const magicScript = `
<script>
// MAGIC MAP LISTENER
...
if (!html.includes('MAGIC MAP LISTENER')) {
    html = html.replace('</head>', magicScript + '</head>');
}

// 3. Just clean up the inline onclick just in case, though the global listener will intercept it.
html = html.replace(/onclick="document\.getElementById\('globalMapModal'\).*?renderGlobalLiveMap\(\);"/, '');
html = html.replace(/onclick="openGlobalLiveMap\(\)"/, '');
